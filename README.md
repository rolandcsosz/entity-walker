...
```ts
graph.transaction("tx1").subcategory().mainCategory().value()?.name;
```
...
// Forward traversal
const categoryName = graph.transaction("tx1").subcategory().mainCategory().value()?.name; // "Food"

// Reverse traversal
const txIds = graph.mainCategory("cat1").subcategoryNodes().transactionNodes().ids(); // ["tx1"]
```
