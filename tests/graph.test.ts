...
function runEntityGraphTests(label: string, { rootNode, nodeList, path, makeGraph }: GraphWrapper) {
    ...
    it("scoped() does not affect intermediate traversal nodes", () => {
        ...
        expect(subs).toHaveLength(1);
        expect(subs.entities()).toHaveLength(0);
    });
    ...
    it("scoped() affects isEmpty() and isNotEmpty()", () => {
        ...
        expect(subs.isNotEmpty()).toBe(false);
    });
    ...
    it("scoped() nested scopes AND-compose", () => {
        // First scope: tx1, tx2, tx3
        const first = nodeList("transaction").scoped();
        // Second scope after where: tx1 only
        const second = first.where((t: any) => t.id === "tx1").scoped();
        // Traverse out and back — scope restricts to {tx1}
